...
function getRequiredImports(elements: WatchElement[]): string[] {
  ...
  if (types.has('heart_rate') || types.has('steps') || types.has('body_battery') || types.has('distance')) {
...
function generateElementCode(el: WatchElement): string {
  ...
  switch (el.type) {
    ...
    case 'alarm':
      return `
        // Alarm
        var alarmCount = System.getClockTime().dst;
...
export function generateManifest(layout: WatchfaceLayout, cn: string): string {
  ...
  layout.elements.forEach((el) => {
    if (['heart_rate', 'steps', 'body_battery', 'distance'].includes(el.type)) {
...
## Widgets
${layout.elements.map((el) => `- ${el.type.replace('_', ' ')}`).join('\n')}
